...
function createComparison() {
    ...
    var countries = global_selected.filter(function(d){
        debugger;
    }),
        options = ["sepal length", "petal length", "sepal width", "petal width"];
...
function path(d) {
    return line(options.map(function(p) { return [x(p), y[p](d[p])]; }));
}

// Handles a brush event, toggling the display of foreground lines.
function brush() {
    var actives = options.filter(function(p) { return !y[p].brush.empty(); }),
