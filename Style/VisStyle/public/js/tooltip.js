...
function map_tooltip(country_code, start_year, end_year) {
    ...
    var country = country_data.filter(function (d) {
        return d.Code.toLowerCase() === country_code.toLowerCase();
    })[0];
    ...
    FuelTypesSVG.selectAll("rect")
        ...
        .attr("height", function () {
            return (svgHeight - yAxis - 5) / FuelTypeOptions.length;
        })
        ...
        .attr("x", xAxis)
        .attr("y", function (d, i) {
            return (svgHeight - yAxis - 5) / FuelTypeOptions.length * i;
        })
        ...
        .style("fill", function (d) {
            return ColorScale(d3.sum(d.Years) / d.Years.length)
        })
        .selectAll("text")
        .data(function (d) {
            return d;
    ...
    FuelTypesSVG.append("g")
        ...
        .attr("x", xAxis - 5)
        .attr("y", function (d, i) {
            return (svgHeight - yAxis) / FuelTypeOptions.length * (i + 1);
        })
    ...
    CombustionFactorsSVG.selectAll("rect")
        ...
        .attr("transform", "translate(" + 0 + "," + (15) + ")")
        .attr("height", function () {
            return (svgHeight - yAxis - 5) / CombustionFactorOptions.length;
        })
        .attr("width", function (d) {
            var w = xScale(d3.sum(d.Years) / d.Years.length);
            if (isNaN(w) || w < 0)
        ...
        .attr("x", xAxis)
        .attr("y", function (d, i) {
            return (svgHeight - yAxis - 5) / CombustionFactorOptions.length * i;
        })
        .attr("class", function (d, i) {
            if (i % 2 == 0)return "bar";
            return "bar";
    ...
    CombustionFactorsSVG.append("g")
        ...
        .attr("x", xAxis - 5)
        .attr("y", function (d, i) {
            return (svgHeight - yAxis + 5 ) / CombustionFactorOptions.length * (i + 1);
        })
        .attr("text-anchor", "end")
        .text(function (d) {
            switch (d.Name) {
